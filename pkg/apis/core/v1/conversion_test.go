...
// TestPodConversionAliasesMemory asserts Pod and PodList conversions are zero-copy.
// The gate of record is +k8s:conversion-gen:memory-identical-to in
// pkg/apis/core/types.go; this test only catches runtime conversion registrations
// that shadow the generated functions, which the generator cannot see.
func TestPodConversionAliasesMemory(t *testing.T) {
	...
	// The PodList Items cast never invokes the Pod pair's conversion functions, so
	// shadowing of those is probed separately via a contained slice and the
	// annotations map (which the historical hand-written Pod conversion rewrote).
