...
type preexistingExpressions struct {
	...
	jsonPatchValueExpressions        sets.Set[string]
	jsonPatchPathExpressions         sets.Set[string]
}
...
func newPreexistingExpressions() preexistingExpressions {
	return preexistingExpressions{
		...
		jsonPatchValueExpressions:        sets.New[string](),
		jsonPatchPathExpressions:         sets.New[string](),
	}
}
...
func findMutatingPolicyPreexistingExpressions(mutatingPolicy *admissionregistration.MutatingAdmissionPolicy) preexistingExpressions {
	...
	for _, v := range mutatingPolicy.Spec.Mutations {
		...
		for _, patch := range v.JSONPatch {
			preexisting.jsonPatchValueExpressions.Insert(patch.ValueExpression)
		}
	}
	return preexisting
}
...
var supportedJSONPatchOperation = sets.NewString(
	string(admissionregistration.Add),
	string(admissionregistration.Remove),
	string(admissionregistration.Replace),
	string(admissionregistration.Copy),
	string(admissionregistration.Move),
	string(admissionregistration.Test),
)

// MutatatingAdmissionPolicy does not support DELETE or CONNECT
...
func validateMutation(compiler plugincel.Compiler, m *admissionregistration.Mutation, paramKind *admissionregistration.ParamKind, opts validationOptions, fldPath *field.Path) (allErrors field.ErrorList) {
	if len(m.ReinvocationPolicy) == 0 {
		allErrors = append(allErrors, field.Required(fldPath.Child("reinvocationPolicy"), ""))
	} else if !supportedReinvocationPolicies.Has(string(m.ReinvocationPolicy)) {
...
func validateApplyConfiguration(compiler plugincel.Compiler, applyConfig *admissionregistration.ApplyConfiguration, paramKind *admissionregistration.ParamKind, opts validationOptions, fldPath *field.Path) (allErrors field.ErrorList) {
	...
	if len(trimmedExpression) == 0 {
		...
		opts := plugincel.OptionalVariableDeclarations{HasParams: paramKind != nil, HasAuthorizer: true, StrictCost: true, HasObjectTypes: true}
...
func validateJSONPatch(compiler plugincel.Compiler, jsonPatch admissionregistration.JSONPatch, paramKind *admissionregistration.ParamKind, opts validationOptions, fldPath *field.Path) (allErrors field.ErrorList) {
	for i, patch := range jsonPatch {
		trimmedValueExpression := strings.TrimSpace(patch.ValueExpression)
		hasValue := len(trimmedValueExpression) > 0
		trimmedFromExpression := strings.TrimSpace(patch.FromExpression)
		hasFrom := len(trimmedFromExpression) > 0
		needValue := false
		needFrom := false
		switch patch.Op {
		case admissionregistration.Add, admissionregistration.Replace, admissionregistration.Test:
			needValue = true
		case admissionregistration.Move, admissionregistration.Copy:
			needFrom = true
		case admissionregistration.Remove:
		default:
			allErrors = append(allErrors, field.NotSupported(fldPath.Index(i).Child("op"), patch.Op, supportedJSONPatchOperation.List()))
		}

		trimmedPathExpression := strings.TrimSpace(patch.PathExpression)
		if len(trimmedPathExpression) == 0 {
			allErrors = append(allErrors, field.Required(fldPath.Index(i).Child("pathExpression"), ""))
		} else {
			envType := environment.NewExpressions
			if opts.preexistingExpressions.jsonPatchValueExpressions.Has(patch.ValueExpression) {
				envType = environment.StoredExpressions
			}
			accessor := &mutating.JSONPatchPathCondition{
				Expression: trimmedPathExpression,
			}
			result := compiler.CompileCELExpression(accessor, plugincel.OptionalVariableDeclarations{HasParams: paramKind != nil, HasAuthorizer: true, StrictCost: true}, envType)
			if result.Error != nil {
				allErrors = append(allErrors, convertCELErrorToValidationError(fldPath.Child("pathExpression"), accessor, result.Error))
			}
		}

		if needValue && !hasValue {
			allErrors = append(allErrors, field.Required(fldPath.Index(i).Child("valueExpression"), fmt.Sprintf("must be specified when op is %s", string(patch.Op))))
		}
		if !needValue && hasValue {
			allErrors = append(allErrors, field.Invalid(fldPath.Index(i).Child("valueExpression"), "{valueExpression}", fmt.Sprintf("must not be specified when op is %s", string(patch.Op))))
		}
		if needValue && hasValue {
			envType := environment.NewExpressions
			if opts.preexistingExpressions.jsonPatchValueExpressions.Has(patch.ValueExpression) {
			...
			accessor := &mutating.JSONPatchCondition{
				Expression: trimmedValueExpression,
			}
			result := compiler.CompileCELExpression(accessor, plugincel.OptionalVariableDeclarations{HasParams: paramKind != nil, HasAuthorizer: true, StrictCost: true}, envType)
			if result.Error != nil {
				allErrors = append(allErrors, convertCELErrorToValidationError(fldPath.Child("valueExpression"), accessor, result.Error))
			}
		}

		if needFrom && !hasFrom {
			allErrors = append(allErrors, field.Required(fldPath.Index(i).Child("fromExpression"), fmt.Sprintf("must be specified when op is %s", string(patch.Op))))
		}
		if !needFrom && hasFrom {
			allErrors = append(allErrors, field.Invalid(fldPath.Index(i).Child("fromExpression"), patch.FromExpression, fmt.Sprintf("must not be specified when op is %s", string(patch.Op))))
		}
		if needFrom && hasFrom {
			envType := environment.NewExpressions
			if opts.preexistingExpressions.jsonPatchValueExpressions.Has(patch.FromExpression) {
				envType = environment.StoredExpressions
			}
			accessor := &mutating.JSONPatchPathCondition{
				Expression: trimmedFromExpression,
			}
			result := compiler.CompileCELExpression(accessor, plugincel.OptionalVariableDeclarations{HasParams: paramKind != nil, HasAuthorizer: true, StrictCost: true}, envType)
			if result.Error != nil {
				allErrors = append(allErrors, convertCELErrorToValidationError(fldPath.Child("fromExpression"), accessor, result.Error))
			}
		}
	}
