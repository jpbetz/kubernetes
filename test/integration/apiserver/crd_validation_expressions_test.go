...
// TestCustomResourceValidatorsWithDisabledFeatureGate test that x-kubernetes-validators work as expected when the
...
// TestCustomResourceValidators tests x-kubernetes-validators compile and validate as expected when the feature gate
// is enabled.
	...
	t.Run("Structural schema", func(t *testing.T) {
		...
		t.Run("custom resource create and update MUST NOT allow data that is invalid according to x-kubernetes-validations if the feature gate is enabled", func(t *testing.T) {
			...
			if err == nil {
				...
				if !strings.Contains(err.Error(), "failed rule 'spec.x + spec.y") {
					t.Fatalf("Expected error to contain %s but got %v", "failed rule 'spec.x + spec.y", err.Error())
				}
			}
...
var structuralSchemaWithValidators = []byte(`
{
  "openAPIV3Schema": {
	...
	"x-kubernetes-validations": [
	  {
		"rule": "spec.x + spec.y >= (has(self.status) ? self.status.z : 0)"
	  }
	],
    "properties": {
      "spec": {
