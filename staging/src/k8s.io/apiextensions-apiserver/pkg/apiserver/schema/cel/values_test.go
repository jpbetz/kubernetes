...
func TestMapper(t *testing.T) {
	cases := []struct {
		...
		schema         *schema.Structural
		propertySchema func(key string) *schema.Structural
		size           int64
		notContains    []ref.Val
	}{
		{
			...
			schema:         &objectSchema,
			propertySchema: func(key string) *schema.Structural { s := objectSchema.Properties[key]; return &s },
			size:           2,
			notContains: []ref.Val{
		...
		},
		{
			...
			schema:         &mapSchema,
			propertySchema: func(key string) *schema.Structural { return mapSchema.AdditionalProperties.Structural },
			size:           2,
			notContains: []ref.Val{
