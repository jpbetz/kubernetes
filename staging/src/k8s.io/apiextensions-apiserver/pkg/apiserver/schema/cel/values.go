	...
	if schema.Type == "object" {
		...
		if schema.AdditionalProperties != nil && schema.AdditionalProperties.Structural != nil {
			return &unstructuredMap{
				value:      m,
				schema:     schema,
				propSchema: func(key string) *structuralschema.Structural { return schema.AdditionalProperties.Structural },
			}
	...
	if schema.Type == "array" {
		...
		if listType != nil {
			...
			case "map":
				...
				return &unstructuredMapList{unstructuredList: typedList, escapedKeyProps: mapKeys}
			case "set":
...
type unstructuredMapList struct {
	unstructuredList
	escapedKeyProps []string // escaped
...
func (t *unstructuredMapList) Equal(other ref.Val) ref.Val {
	...
	for it := oMapList.Iterator(); it.HasNext() == types.True; {
		...
		if UnstructuredToVal(tVal, t.itemsSchema).Equal(v) != types.True {
			return types.False
		}
	}
	return types.True
...
func (t *unstructuredMapList) Add(other ref.Val) ref.Val {
	...
	return &unstructuredMapList{
		...
		escapedKeyProps:  model.EscapeSlice(t.escapedKeyProps),
	}
}
...
type unstructuredMap struct {
	...
	propSchema func(key string) *structuralschema.Structural
}
...
func (t *unstructuredMap) Iterator() traits.Iterator {
	...
	i := 0
	for k := range t.value {
		keys[i] = types.String(model.Escape(k)) // TODO: only for objects
		i++
	}
...
func (t *unstructuredMap) Find(key ref.Val) (ref.Val, bool) {
	escapedKeyStr, ok := key.(types.String)
	if !ok {
		return types.MaybeNoSuchOverloadErr(key), true
	}
	k := escapedKeyStr.Value().(string)
	uk := model.Unescape(k)
	if v, ok := t.value[uk]; ok {
		return UnstructuredToVal(v, t.propSchema(uk)), true
	}
