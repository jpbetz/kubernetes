...
func buildSubresourceDefaultVerbTemplates(generateApply bool) map[string]string {
	...
	if generateApply {
		m["apply"] = `Apply(ctx context.Context, $.type|private$Name string, $.inputType|private$ *$.applyConfig|raw$, options $.ApplyOptions|raw$) (result *$.resultType|raw$, err error)`
	}
	return m
}
...
func (c *$.type|privatePlural$) Apply(ctx context.Context, $.type|private$Name string, $.inputType|private$ *$.applyConfig|raw$, opts $.ApplyOptions|raw$) (result *$.resultType|raw$, err error) {
