...
var (
	...
	visitPkg            = "k8s.io/apimachinery/pkg/api/validate/visit"
	visitPkgSymbols     = mkPkgNames(visitPkg, "Child", "Leaf", "Wrap", "State", "NewState")
)
...
func (g *genValidations) emitRegisterFunction(c *generator.Context, schemeRegistry types.Name, sw *generator.SnippetWriter) {
	...
	for _, rootType := range g.rootTypes {
		...
		targs := generator.Args{
			...
			"visit":       mkSymbolArgs(c, visitPkgSymbols),
		}
		...
		sw.Do("               $.safe.Cast|raw$[$.typePfx$$.rootType|raw$](oldObj), visit.NewState())\n", targs)
		sw.Do("  }\n", targs)
...
func (g *genValidations) emitValidationFunction(c *generator.Context, t *types.Type, sw *generator.SnippetWriter) {
	...
	sw.Do("    obj, oldObj $.objTypePfx$$.inType|raw$, v *$.visit.State|raw$) ", targs)
	...
	case types.Struct:
		for _, fld := range thisNode.fields {
			...
			if buf.Len() > 0 {
				...
				// FIXME: Is this the right way to identify leaf nodes?
				leafFn := types.Name{Package: visitPkg, Name: "Child"}
				if fld.node == nil || len(fld.node.fields) == 0 {
					leafFn = types.Name{Package: visitPkg, Name: "Leaf"}
				}
				...
				targs := targs.WithArgs(generator.Args{
					...
					"visit":        mkSymbolArgs(c, visitPkgSymbols),
					"visitFn":      c.Universe.Type(leafFn),
				})
				...
				sw.Do("  $.visitFn|raw$(op, v, func(fldPath *$.field.Path|raw$, obj, oldObj $.fieldTypePfx$$.fieldType|raw$) (errs $.field.ErrorList|raw$) {\n", targs)
...
func (g *genValidations) emitCallToOtherTypeFunc(c *generator.Context, node *typeNode, sw *generator.SnippetWriter) {
	...
	sw.Do("errs = append(errs, $.funcName|raw$(ctx, op, fldPath, obj, oldObj, v)...)\n", targs)
}
...
func toGolangSourceDataLiteral(sw *generator.SnippetWriter, c *generator.Context, value any) {
	...
	case validators.WrapperFunction:
		extraArgs := v.Function.Args
		// FIXME: need a better way of identifying generated functions
		if strings.HasPrefix(v.Function.Function.Name, "Validate_") {
			extraArgs = append(extraArgs, validators.PrivateVar{Name: "v", Package: "local"})
		}
		if len(extraArgs) == 0 {
