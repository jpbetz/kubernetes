...
func (g *genValidations) emitValidationForChild(c *generator.Context, thisChild *childNode, sw *generator.SnippetWriter) {
	...
	case types.Struct:
		for _, fld := range thisNode.fields {
			...
			if buf.Len() > 0 {
				...
				childIsNilable := isNilableType(fld.childType)
				if !childIsNilable {
					targs["maybeAddr"] = "&"
					targs["maybePtr"] = "*"
				}
				...
				sw.Do("  func(obj $.maybePtr$$.fieldType|raw$, fldPath *$.fieldPath|raw$) (errs $.errorList|raw$) {\n", targs)
				sw.Append(buf)
				sw.Do("    return\n", targs)
				sw.Do("  }($.maybeAddr$obj.$.fieldName$, fldPath.Child(\"$.fieldJSON$\"))...)\n", targs)
				sw.Do("\n", nil)
			} else {
				targs := targs.WithArgs(generator.Args{
					"fieldName": fld.name,
	...
	case types.Slice, types.Array:
		...
		if elemBuf.Len() > 0 {
			...
			elemIsNilable := isNilableType(inType.Elem)
			if !elemIsNilable {
				targs["maybeAddr"] = "&"
				targs["maybePtr"] = "*"
			}
			...
			sw.Do("  errs = append(errs,\n", targs)
			sw.Do("    func(obj $.maybePtr$$.elemType|raw$, fldPath *$.fieldPath|raw$) (errs $.errorList|raw$) {\n", targs)
			sw.Append(elemBuf)
			sw.Do("      return\n", targs)
			sw.Do("    }($.maybeAddr$val, fldPath.Index(i))...)\n", targs)
			sw.Do("}\n", nil)
		}
	case types.Map:
		// Accumulate into a buffer so we don't emit empty functions.
		...
		if keyBuf.Len()+valBuf.Len() > 0 {
			...
			keyIsNilable := isNilableType(inType.Key)
			if !keyIsNilable {
				targs["keyMaybeAddr"] = "&"
				targs["keyMaybePtr"] = "*"
			}
			valIsNilable := isNilableType(inType.Elem)
			if !valIsNilable {
				targs["valMaybeAddr"] = "&"
				targs["valMaybePtr"] = "*"
			}
			...
			if keyBuf.Len() > 0 {
				sw.Do("  errs = append(errs,\n", targs)
				sw.Do("    func(obj $.keyMaybePtr$$.keyType|raw$, fldPath *$.fieldPath|raw$) (errs $.errorList|raw$) {\n", targs)
				sw.Append(keyBuf)
				sw.Do("      return\n", targs)
				sw.Do("    }($.keyMaybeAddr$key, fldPath)...)\n", targs)
			}
			if valBuf.Len() > 0 {
				sw.Do("  errs = append(errs,\n", targs)
				sw.Do("    func(obj $.valMaybePtr$$.valType|raw$, fldPath *$.fieldPath|raw$) (errs $.errorList|raw$) {\n", targs)
				sw.Append(valBuf)
				sw.Do("      return\n", targs)
				sw.Do("    }($.valMaybeAddr$val, fldPath.Key(key))...)\n", targs) // TODO: what if the key is not a string?
			}
			sw.Do("}\n", nil)
		}
	default:
...
func (g *genValidations) emitCallToOtherTypeFunc(c *generator.Context, node *typeNode, sw *generator.SnippetWriter) {
	...
	sw.Do("errs = append(errs, $.funcName|raw$(obj, fldPath)...)\n", targs)
}
