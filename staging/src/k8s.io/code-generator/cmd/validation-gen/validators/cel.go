...
func (stv ruleTagValidator) GetValidations(context Context, args []string, payload string) (Validations, error) {
	...
	if p.Reason == nil {
		p.Reason = pointer.To(metav1.StatusReasonInvalid)
	} else {
		if p.Reason != nil && !supportedPolicyReason.Has(*p.Reason) {
	...
	supportVar := Variable(supportVarName, Function(ruleTagName, DefaultFlags, newRule, p.Expression, p.MessageExpression, p.Message, reason))
