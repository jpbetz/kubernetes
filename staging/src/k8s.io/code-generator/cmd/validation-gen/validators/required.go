...
func (rtv requirednessTagValidator) hasZeroDefault(context Context) (bool, bool, error) {
	...
	payload, hasDefault := gengo.ExtractCommentTags("+", context.Member.CommentLines)[defaultTagName]
	if !hasDefault {
	...
	if len(payload) > 1 {
		return false, false, fmt.Errorf("default tag with multiple values: %q", payload)
	}

	var defaultValue any
	if err := json.Unmarshal([]byte(payload[0]), &defaultValue); err != nil {
