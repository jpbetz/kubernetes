...
func (frtv fixedResultTagValidator) GetValidations(context Context, tag codetags.Tag) (Validations, error) {
	...
	payloadTag, err := frtv.toFixedResultArgs(tag)
	if err != nil {
		return result, fmt.Errorf("can't decode tag payload: %w", err)
	}
	result.AddFunction(Function(frtv.TagName(), payloadTag.flags, fixedResultValidator, frtv.result, payloadTag.msg).WithTypeArgs(payloadTag.typeArgs...))
