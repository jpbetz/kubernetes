...
type resourceExpirationEvaluator struct {
	emulationVersion *apimachineryversion.Version
	isAlpha          bool
...
func NewResourceExpirationEvaluator(emulationVersion string) (ResourceExpirationEvaluator, error) {
	if len(emulationVersion) == 0 {
		return nil, fmt.Errorf("empty NewResourceExpirationEvaluator emulationVersion")
	}
	klog.V(1).Infof("NewResourceExpirationEvaluator with emulationVersion: %s.", emulationVersion)
	ret := &resourceExpirationEvaluator{
		strictRemovedHandlingInAlpha: false,
	}
	parseVersion, err := apimachineryversion.ParseMajorMinor(emulationVersion)
	if err != nil {
		return nil, err
	}
	// Only keeps the major and minor versions from the version string.
	ret.emulationVersion = parseVersion

	ret.isAlpha = strings.Contains(emulationVersion, "alpha")
...
func (e *resourceExpirationEvaluator) ShouldServeForVersion(majorRemoved, minorRemoved int) bool {
	...
	if removedVer.GreaterThan(e.emulationVersion) {
		return true
	}
	if removedVer.LessThan(e.emulationVersion) {
		return false
	}
