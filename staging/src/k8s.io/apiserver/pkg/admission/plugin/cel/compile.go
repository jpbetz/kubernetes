...
func mustBuildEnvs(baseEnv *environment.EnvSet) variableDeclEnvs {
	...
	for _, hasParams := range []bool{false, true} {
		for _, hasAuthorizer := range []bool{false, true} {
			...
			decl := OptionalVariableDeclarations{HasParams: hasParams, HasAuthorizer: hasAuthorizer, StrictCost: true, HasObjectTypes: true}
			envs[decl] = createEnvForOpts(baseEnv, namespaceType, requestType, decl)
		}
	}
	return envs
...
func createEnvForOpts(baseEnv *environment.EnvSet, namespaceType *apiservercel.DeclType, requestType *apiservercel.DeclType, opts OptionalVariableDeclarations) *environment.EnvSet {
	...
	if opts.HasObjectTypes {
		extended, err = extended.Extend(hasObjectTypesOpt)
		if err != nil {
			panic(fmt.Sprintf("environment misconfigured: %v", err))
		}
	}
...
var hasObjectTypesOpt = environment.VersionedOptions{
