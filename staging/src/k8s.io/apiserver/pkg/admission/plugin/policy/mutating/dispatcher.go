...
func NewDispatcher(a authorizer.Authorizer, m *matching.Matcher, tcm patch.TypeConverterManager, resolver resolver.SchemaResolver) generic.Dispatcher[PolicyHook] {
	res := &dispatcher{
		matcher:              m,
		authz:                a,
...
func (d *dispatcher) dispatchInvocations(
	...
	for _, invocation := range invocations {
		...
		for mutationIndex, mutation := range invocation.Policy.Spec.Mutations {
			...
			changed, err := d.dispatchOne(ctx, patcher, o, versionedAttr, namespace, invocation.Resource, optionalVariables)
			if err != nil {
			...
			if mutation.ReinvocationPolicy == v1alpha1.IfNeededReinvocationPolicy {
...
func (d *dispatcher) dispatchOne(
	...
	if typeConverter == nil {
		...
		return false, k8serrors.NewServiceUnavailable(fmt.Sprintf("Resource kind %s not found. There can be a delay between when CustomResourceDefinitions are created and when they are available.", versionedAttributes.VersionedKind))
	}
