...
var structSchema = &spec.Schema{
	SchemaProps: spec.SchemaProps{
		...
		Properties: map[string]spec.Schema{
			...
			"i": *int64Schema,
			"b": *boolSchema,
			"f": {SchemaProps: spec.SchemaProps{Type: []string{"number"}}},
		},
	},
}
...
func (s StructOmitEmpty) GetObjectKind() schema.ObjectKind {
	panic("not implemented")
}

func (s StructOmitEmpty) DeepCopyObject() runtime.Object {
	panic("not implemented")
}
...
var (
	...
	stringArraySchema = spec.Schema{SchemaProps: spec.SchemaProps{Type: []string{"array"}, Items: &spec.SchemaOrArray{Schema: stringSchema}}}
	intArraySchema    = spec.Schema{SchemaProps: spec.SchemaProps{Type: []string{"array"}, Items: &spec.SchemaOrArray{Schema: int64Schema}}}
	stringMapSchema   = spec.Schema{SchemaProps: spec.SchemaProps{Type: []string{"object"}, AdditionalProperties: &spec.SchemaOrBool{Schema: stringSchema}}}
	intMapSchema      = spec.Schema{SchemaProps: spec.SchemaProps{Type: []string{"object"}, AdditionalProperties: &spec.SchemaOrBool{Schema: int64Schema}}}
)
...
var complexSchema = &spec.Schema{
	SchemaProps: spec.SchemaProps{
		...
		Properties: map[string]spec.Schema{
			...
			"nilMap":     intMapSchema,
			"intOrString": {
				VendorExtensible: spec.VendorExtensible{Extensions: map[string]interface{}{"x-kubernetes-int-or-string": true}},
			},
...
func typedToValActivation(vals map[string]typedValue) map[string]interface{} {
	...
	for k, tv := range vals {
		activation[k] = common.TypedToVal(tv.value)
	}
	return activation
}
...
func unstructuredToValActivation(vals map[string]typedValue) (map[string]interface{}, error) {
	activation := make(map[string]interface{}, len(vals))
	for k, tv := range vals {
		schema := &openapi.Schema{tv.schema}
		...
		case runtime.Object:
			...
			activation[k] = common.UnstructuredToVal(u, schema)
		case *runtime.Object:
			u, err := runtime.DefaultUnstructuredConverter.ToUnstructured(v)
			if err != nil {
				return nil, err
		...
		default:
			...
			if uv, ok := u["value"]; ok && uv != nil {
				activation[k] = common.UnstructuredToVal(uv, schema)
			} else {
...
func TestTypedToVal(t *testing.T) {
	...
	tests := []testCase{
		...
		},
		{
			...
			activation: map[string]typedValue{"obj": typedValue{value: structOmitEmpty1, schema: structSchema}},
		},
		{
		...
		},
		{
			...
			activation: map[string]typedValue{"c": typedValue{value: structOmitEmpty1, schema: structSchema}},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Test that TypedToVal's behaves in an activation in the same way as raw unstructured.
			// Admission policies use raw unstructured.
