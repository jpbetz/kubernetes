...
func TestListAdd(t *testing.T) {
	...
	tests := []struct {
		...
		{testCase: testCase{
			name:       "serialized set elements distinguish value boundaries on union",
			expression: "size(x.setOfLists + y.setOfLists) == 3",
		}, skipSchemaless: true},
		{testCase: testCase{
			name:       "serialized set element equality ignores element order",
			expression: "x.setOfLists == z.setOfLists && z.setOfLists == x.setOfLists",
		}, skipSchemaless: true},
		{testCase: testCase{
			name:       "set lists with shifted value boundaries are not equal",
			expression: "x.setOfLists != y.setOfLists && y.setOfLists != x.setOfLists",
		}, skipSchemaless: true},
		{testCase: testCase{
			name:       "set list of lists unions literal elements",
			expression: "size(x.setOfLists + [['x']]) == 2 && (x.setOfLists + [['x']]) == x.setOfLists",
		}, skipSchemaless: true},
		{testCase: testCase{
			name:       "set list of lists appends distinct literal elements",
			expression: "size(x.setOfLists + [['a', 'b c']]) == 3",
		}, skipSchemaless: true},
		{testCase: testCase{
			name:       "set list of lists equality with literal list",
			expression: "x.setOfLists == [['a b', 'c'], ['x']] && (x.setOfLists + []) == [['a b', 'c'], ['x']]",
		}, skipSchemaless: true},
		{testCase: testCase{
			name:       "concatenated set list of lists keeps union semantics",
			expression: "((x.setOfLists + [['q']]) + [['q'], ['x']]) == (x.setOfLists + [['q']])",
		}, skipSchemaless: true},
		{testCase: testCase{
