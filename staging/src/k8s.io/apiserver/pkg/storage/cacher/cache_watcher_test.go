...
func TestCacheWatcherCleanupNotBlockedByResult(t *testing.T) {
	...
	// w.result is blocked.
	w = newCacheWatcher(0, filter, forget, storage.APIObjectVersioner{}, time.Now(), false, schema.GroupResource{Resource: "pods"}, func() string { return "" })
...
func TestCacheWatcherStoppedInAnotherGoroutine(t *testing.T) {
	...
	for i := 0; i < maxRetriesToProduceTheRaceCondition; i++ {
		w = newCacheWatcher(2, filter, emptyFunc, storage.APIObjectVersioner{}, deadline, false, schema.GroupResource{Resource: "pods"}, func() string { return "" })
...
func TestResourceVersionAfterInitEvents(t *testing.T) {
	...
	w := newCacheWatcher(numObjects+1, filter, forget, storage.APIObjectVersioner{}, deadline, true, schema.GroupResource{Resource: "pods"}, func() string { return "" })
...
func TestTimeBucketWatchersBasic(t *testing.T) {
	...
	newWatcher := func(deadline time.Time) *cacheWatcher {
		w := newCacheWatcher(0, filter, forget, storage.APIObjectVersioner{}, deadline, true, schema.GroupResource{Resource: "pods"}, func() string { return "" })
...
func TestCacheWatcherDraining(t *testing.T) {
	...
	w = newCacheWatcher(1, filter, forget, storage.APIObjectVersioner{}, time.Now(), true, schema.GroupResource{Resource: "pods"}, func() string { return "" })
...
func TestCacheWatcherDrainingNoBookmarkAfterResourceVersionReceived(t *testing.T) {
	...
	w = newCacheWatcher(0, filter, forget, storage.APIObjectVersioner{}, time.Now(), true, schema.GroupResource{Resource: "pods"}, func() string { return "" })
...
func TestCacheWatcherDrainingNoBookmarkAfterResourceVersionSent(t *testing.T) {
	...
	w = newCacheWatcher(2, filter, forget, storage.APIObjectVersioner{}, time.Now(), true, schema.GroupResource{Resource: "pods"}, func() string { return "" })
	w.setBookmarkAfterResourceVersion(10)
...
func TestBookmarkAfterResourceVersionWatchers(t *testing.T) {
	newWatcher := func(id string, deadline time.Time) *cacheWatcher {
		w := newCacheWatcher(0, func(_ string, _ labels.Set, _ fields.Set, _ runtime.Object) bool { return true }, func(bool) {}, storage.APIObjectVersioner{}, deadline, true, schema.GroupResource{Resource: "pods"}, func() string { return id })
		w.setBookmarkAfterResourceVersion(10)
		return w
	}
