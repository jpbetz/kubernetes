...
func TestDeleteTriggerWatch(t *testing.T) {
	...
	w, err := store.Watch(ctx, key, storage.MinimumRV(storedObj.ResourceVersion), storage.Everything)
	if err != nil {
		t.Fatalf("Watch failed: %v", err)
	}
...
func TestWatchFromZero(t *testing.T) {
	...
	w, err := store.Watch(ctx, key, storage.MinimumRV("0"), storage.Everything)
	if err != nil {
		t.Fatalf("Watch failed: %v", err)
	}
...
func TestWatchError(t *testing.T) {
	...
	w, err := invalidStore.Watch(ctx, "/abc", storage.MinimumRV("0"), storage.Everything)
	if err != nil {
		t.Fatalf("Watch failed: %v", err)
	}
