...
var map_Lookup = map[string]string{
	"id":         "The CEL field name of the looked up resource.",
	"apiVersion": "The API version of the resource to look up.",
	"kind":       "The kind of the resource to look up.",
	"name":       "An expression that evaluates to the name of the resource to look up.",
	"namespace":  "An expression that evaluates to the namespace of the resource to look up.",
}

func (Lookup) SwaggerDoc() map[string]string {
	return map_Lookup
}
...
var map_ValidatingRule = map[string]string{
	"":                  "ValidatingRule describes a validation rule and the resources and operations it applies to.",
	"name":              "The name of the admission rule. Name should be fully qualified, e.g., imagepolicy.kubernetes.io, where \"imagepolicy\" is the name of the rule, and kubernetes.io is the name of the organization. Required.",
	"validations":       "validations defines the validation rules checked by this admission rule.",
	"lookups":           "lookups defines which additional resources are available to the validation rule's CEL expressions.",
	"matchRules":        "matchRules describes what operations on what resources/subresources the webhook cares about. The webhook cares about an operation if it matches _any_ Rule. However, in order to prevent ValidatingAdmissionWebhooks and MutatingAdmissionWebhooks from putting the cluster in a state which cannot be recovered from without completely disabling the plugin, ValidatingAdmissionWebhooks and MutatingAdmissionWebhooks are never called on admission requests for ValidatingWebhookConfiguration and MutatingWebhookConfiguration objects.",
	"matchPolicy":       "matchPolicy defines how the \"rules\" list is used to match incoming requests. Allowed values are \"Exact\" or \"Equivalent\".\n\n- Exact: match a request only if it exactly matches a specified rule. For example, if deployments can be modified via apps/v1, apps/v1beta1, and extensions/v1beta1, but \"rules\" only included `apiGroups:[\"apps\"], apiVersions:[\"v1\"], resources: [\"deployments\"]`, a request to apps/v1beta1 or extensions/v1beta1 would not be sent to the rules.\n\n- Equivalent: match a request if modifies a resource listed in rules, even via another API group or version. For example, if deployments can be modified via apps/v1, apps/v1beta1, and extensions/v1beta1, and \"rules\" only included `apiGroups:[\"apps\"], apiVersions:[\"v1\"], resources: [\"deployments\"]`, a request to apps/v1beta1 or extensions/v1beta1 would be converted to apps/v1 and sent to the rules.\n\nDefaults to \"Equivalent\"",
	"namespaceSelector": "NamespaceSelector decides whether to run the rule on an object based on whether the namespace for that object matches the selector. If the object itself is a namespace, the matching is performed on object.metadata.labels. If the object is another cluster scoped resource, it never skips the rule.\n\nFor example, to run the rules on any objects whose namespace is not associated with \"runlevel\" of \"0\" or \"1\";  you will set the selector as follows: \"namespaceSelector\": {\n  \"matchExpressions\": [\n    {\n      \"key\": \"runlevel\",\n      \"operator\": \"NotIn\",\n      \"values\": [\n        \"0\",\n        \"1\"\n      ]\n    }\n  ]\n}\n\nIf instead you want to only run the rules on any objects whose namespace is associated with the \"environment\" of \"prod\" or \"staging\"; you will set the selector as follows: \"namespaceSelector\": {\n  \"matchExpressions\": [\n    {\n      \"key\": \"environment\",\n      \"operator\": \"In\",\n      \"values\": [\n        \"prod\",\n        \"staging\"\n      ]\n    }\n  ]\n}\n\nSee https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/ for more examples of label selectors.\n\nDefault to the empty LabelSelector, which matches everything.",
	"objectSelector":    "ObjectSelector decides whether to run the rules based on if the object has matching labels. objectSelector is evaluated against both the oldObject and newObject that would be sent to the rules, and is considered to match if either object matches the selector. A null object (oldObject in the case of create, or newObject in the case of delete) or an object that cannot have labels (like a DeploymentRollback or a PodProxyOptions object) is not considered to match. Use the object selector only if the rules are opt-in, because end users may skip the admission rules by setting the labels. Default to the empty LabelSelector, which matches everything.",
}

func (ValidatingRule) SwaggerDoc() map[string]string {
	return map_ValidatingRule
}

var map_ValidatingRuleConfiguration = map[string]string{
	...
	"metadata":        "Standard object metadata; More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata.",
	"validatingRules": "validatingRules is a list of validation rules.",
}

func (ValidatingRuleConfiguration) SwaggerDoc() map[string]string {
	return map_ValidatingRuleConfiguration
}

var map_ValidatingRuleConfigurationList = map[string]string{
	"":         "ValidatingRuleConfigurationList is a list of ValidatingRuleConfiguration.",
	"metadata": "Standard list metadata. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds",
	"items":    "List of ValidatingRuleConfiguration.",
}

func (ValidatingRuleConfigurationList) SwaggerDoc() map[string]string {
	return map_ValidatingRuleConfigurationList
}
