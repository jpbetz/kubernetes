...
var map_JSONPatchOperation = map[string]string{
	"":                "JSONPatchOperation defines a JSON Patch operation.\n\nJSONPatchOperation contains multiple fields which are declared as CEL expressions. ref: https://github.com/google/cel-spe CEL expressions have access to the contents of the API request/response, organized into CEL variables as well as some other useful variables:\n\n  - 'object' - The object from the incoming request. The value is null for DELETE requests.\n  - 'oldObject' - The existing object. The value is null for CREATE requests.\n  - 'request' - Attributes of the API request([ref](/pkg/apis/admission/types.go#AdmissionRequest)).\n  - 'params' - Parameter resource referred to by the policy binding being evaluated. Only populated if the policy has a ParamKind.\n  - 'namespaceObject' - The namespace object that the incoming object belongs to. The value is null for cluster-scoped resources.\n  - 'variables' - Map of composited variables, from its name to its lazily evaluated value.\n    For example, a variable named 'foo' can be accessed as 'variables.foo'.\n\nThe `apiVersion`, `kind`, `metadata.name` and `metadata.generateName` are always accessible from the root of the object. No other metadata properties are accessible.\n\nOnly property names of the form `[a-zA-Z_.-/][a-zA-Z0-9_.-/]*` are accessible. Required if the op is \"add\", \"replace\" or \"test\"; must not be set for other op types.",
	"op":              "op specifies the operation this patch performs. Required.",
	"pathExpression":  "pathExpression will be evaluated by CEL to create a path to the target value. path must evaluate to a '/' delimited field path conforming to the [JSON Path spec](https://jsonpatch.com/). Required.",
	"fromExpression":  "fromExpression will be evaluated by CEL to create a path to a value. from must evaluate to a '/' delimited field path conforming to the [JSON Path spec](https://jsonpatch.com/). Required if the op is \"move\" or \"copy\"; must be not be set for other op types.",
	"valueExpression": "valueExpression will be evaluated by CEL to create a value. CEL scalars, maps and lists can be returned, and represent JSON values, maps and arrays when applied by jsonPatch.",
}

func (JSONPatchOperation) SwaggerDoc() map[string]string {
	return map_JSONPatchOperation
}
...
var map_MutatingAdmissionPolicySpec = map[string]string{
	...
	"mutations":        "mutations contain operations which are used to perform mutations. mutations may not be empty; a minimum of one mutation is required. Each mutation represents independent change, is not guaranteed to observe mutations made by preceding mutations, and may be invoked based on the mutation's reinvocationPolicy.",
...
var map_Mutation = map[string]string{
	...
	"reinvocationPolicy": "reinvocationPolicy indicates whether this mutation may be called multiple times as part of a single admission evaluation. Allowed values are \"Never\" and \"IfNeeded\".\n\nNever: the mutation will not be called more than once in a single admission evaluation.\n\nIfNeeded: This mutation may be invoked more than once for a single admission request and there is no guarantee of order with respect to other admission  plugins, admission webhooks, and admission policies.  A mutation is only reinvoked when mutation changes the object after this mutation is invoked. Required.",
	...
	"jsonPatch":          "jsonPatch defines a [JSON patch](https://jsonpatch.com/) operation to perform a mutation to the object. jsonPatch may not be empty; a minimum of one mutation is required. A CEL expression is used to define the patch value.",
}
