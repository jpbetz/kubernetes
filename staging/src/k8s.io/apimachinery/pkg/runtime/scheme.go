...
func (s *Scheme) Validate(ctx context.Context, options sets.Set[string], object Object, resource string, subresources ...string) field.ErrorList {
	if fn, ok := s.validationFuncs[reflect.TypeOf(object)]; ok {
		return fn(ctx, operation.Operation{Type: operation.Create, Request: operation.Request{Resource: resource, Subresources: subresources}, Options: options}, object, nil)
	}
	return nil
}
...
// WARNING: This does not validate all objects!  The handwritten validation code in validation.go
// is not run when this is called.  Only the generated zz_generated.validations.go validation code is run.
func (s *Scheme) ValidateUpdate(ctx context.Context, options sets.Set[string], object, oldObject Object, resource string, subresources ...string) field.ErrorList {
	if fn, ok := s.validationFuncs[reflect.TypeOf(object)]; ok {
		return fn(ctx, operation.Operation{Type: operation.Update, Request: operation.Request{Resource: resource, Subresources: subresources}, Options: options}, object, oldObject)
	}
	return nil
}
